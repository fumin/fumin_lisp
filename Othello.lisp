...
(defvar *win-moves-ht* (make-hash-table :test #'equal)) ;;; ;;; ;;; ;;; ;;; ;;; ;;; ;;; ;;; ;;; ;;; ;;;fumin
  ...
  (let* ((N (length strategies))
    ...
    (dotimes (i N)
      (loop for j from (+ i 1) to (- N 1) do 
          (format t "~&player ~a is playing with player ~a" (elt names i) (elt names j))
...
(load "code:Othello_strategies_fumin.lisp")
(load "code:othello-edgetable.sse2f")